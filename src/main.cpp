...
uint256 hashGenesisBlock("0x7c93c406a8eae49b7636a67e0f39d6327e480f8e9d5e807cd5e308b633c4462d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tipcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tipcoin Signed Message:\n";
...
{
    // Tipcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Tipcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tipcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Tipcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tipcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a6a94eddc18191a06278c77a8e0eafc3ca556aa52effcd111171f7aaf58d10e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "nypost.com 1-26-2018 hackers steal 530m from japanese crypto exchange";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04029b57c210a911077355e624c276") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517094504;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084605379;

        if (fTestNet)
        {
            block.nTime    = 1517094377;
            block.nNonce   = 386136342;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd13f054e127cb9e2bccec54079600f5f98fa33f9695ebcebaa2b1c6ba9625387"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xbb, 0xd3 }; // Tipcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TipcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TipcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TipcoinMiner(CWallet *pwallet)
{
    printf("TipcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tipcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TipcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TipcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TipcoinMiner, pwallet));
}
